...
**0.2 改进版已通过隔离验证，正式服务仍为 0.1，尚未替换。** 改进版为纯模型审阅页：双击数字／字母点标签、拖动旋转、按真实笔迹覆盖的可见表面画笔、撤销重做、草稿恢复和版本锁；对话仅保留在发起审阅的原会话。

## 现在测试

本机服务入口：<http://127.0.0.1:43173/>。这是运行 OpenClaw 的同一台机器的回环地址，不是其他电脑或手机直接可用的网址。

请将地址复制到这台 Mac 的 **Safari／Chrome 独立浏览器地址列**。当前 Control UI 侧边栏「浏览器」访问该地址会报告「当前浏览器导航规则禁止访问此地址」；这与工作台是否在线是不同问题，也不是让用户改模型或重启服务的提示。2026-09-09 再核对首页 HTTP 200、`/api/health` 返回 `ok:true`。Control UI 的「浏览器」与「Portals」是两个不同入口，不能混称已嵌入。

Control UI 的 Portal **尚未创建或验证**。此前创建动作被语音确认机制阻止；关闭语音不等于批准该入口，因此本轮没有重试，也没有修改 Gateway／Control UI 或改为公开网络服务。

以下是 **0.2 激活后** 的操作；当前旧页尚未更新。
...
- [0.2 改进验证与待激活边界](ACCEPTANCE-V02-20260909.md)
