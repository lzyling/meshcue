...
## 1.3.1 · 本批范围（Kelven 2026-09-24 19:05–19:37 定，补丁版）
...
## 待开发 · 审阅者的「上」传不到 Agent（Kelven 2026-09-23 提出，未动工）
...
- 🟠 **颜色只认实体自己的，不继承上层（2026-09-24 实测）。** `toGlb` 只用 occt-import-js 给每个 mesh 的
  ...
  透明度：灯笼这两份 STEP 里 `TRANSPARENCY` 为 0 条——文件里根本没写，alpha 强制为 1 目前没丢信息。
