...
- [ ] **npm —— 结论是「不在 0.14 里」，挪到转 public 之后。** 2026-09-15 查清：
      ...
      **⇒ 建议：不发功能包，也不承诺以后发。** 转公开时先给 git 安装那条路；
      真有人开口要 npm 再发。**发布是单向的，不发是可逆的。**

      ### ⚠️ 但「不发功能包」≠「不占名」—— 这是两件事

      Kelven 09-16 00:43 问「不发会不会被别人恶意占用名字」。**会，而且这个风险是真的。**
      `meshcue`／`meshcue-mcp`／`@lzyling/meshcue` 三个名字 09-16 00:44 复查**仍然全部没人占**，
      任何人现在都能注册。转公开之后项目一旦有人搜，`npm i meshcue` 拿到别人的东西
      就是一条现成的供应链攻击路径 —— 即使我们的文档写的是 `npx github:…`，
      **总有人会凭名字去 npm 试**。

      上面那条「不发」反对的是**功能包的义务**（每版都得能装、semver 不能乱）。
      **占名的 placeholder 没有这些义务**：它不承诺功能，只说明真正的安装方式。
      两件事的代价完全不同，不该被同一句话否掉。

      **⇒ 占名要做，但时机是「转公开那一批」，不是现在**：
      现在发一个叫 `meshcue` 的包，等于**在仓库还私有的时候先把名字和项目的存在公开出去**。

      **转公开时一起办**（列进上面那四件事）：
      · 发一个最小 placeholder 占住 `meshcue` 与 `meshcue-mcp`，内容只说明
        「本项目从 GitHub 安装：`npx github:<owner>/meshcue`」——
        **不要发空壳**，npm 对纯占位包本身也有异议流程；说明真实安装方式的包不算占位。
      · 顺带注册 scope `@<owner>`：scope 归属注册者，别人抢不走；
        但**它保护不了无 scope 的 `meshcue`**，所以两样都要。
      · `"private": true` 会挡住 `npm publish`（这正是它的用途），
        placeholder 要用**单独的最小目录**发，不要为此解掉主包的这道锁。
      · ⚠️ **发布需要 npm 账号 + 2FA，是 Kelven 本人的凭据**，按 GitHub PAT 同一条规矩：
        **CC 不接触，不在 pop 上登录。**

      **万一真被抢先了**：npm 有 name-squatting 异议流程，我们手上有早于对方的
      git 历史、`LICENSE` 署名和公开仓库，证据是够的 —— 但流程慢，且不保证。
      所以这条是「便宜的保险」，不是「可有可无」。
...
- [ ] **永不改名、永不转 org** —— 一旦改名或转移，**旧路径会变成别人可注册的**，
      GitHub 的重定向随即被接管（repojacking）。§0.14 的「同名重建」不触发这条；
      **真要改名之前先回来读这一段。**
...
- [ ] npm 占名 placeholder —— 见 §0.14 那条，转公开同一批做。
