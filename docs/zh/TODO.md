...
## 🗺️ 版本规划（Kelven 2026-09-25 01:13 定）
...
**1.4.0 · GLB 完整支持**（「任何合规 GLB 都能打开，并按作者意图显示」）
...
- A3 morph targets：显示基础形状。⚠️ **现存 bug**：服务端不查 `targets` 放行，查看器 `morphAttributes` 拒 → Agent 以为发布成功、页面报错。
...
- 顺带：文档写明「STL 一律灰，要颜色发 STEP 或 GLB」；库给回的中文零件名乱码；three 升级（先查 0.186 切 STL 后残留 1 张贴图）。

**1.5.0 · 显示保真（B 类）**：动画只显示初始姿势；同一 GLB 里多级 LOD 叠在一起；`KHR_materials_variants` 只显示默认。

**1.6.0 · 游戏美术审阅功能（C 类）**：UV／棋盘格、按通道看（颜色／法线／粗糙度）、分网格面数、节点树显隐。

**2.0.0 · 动画播放**：骨骼动画逐帧播放 —— Mixamo 那类「绑定好骨骼的角色＋动作片段」。
Mixamo 下载只有 FBX／DAE：按「朝向不对由 Agent 先转换」同一原则，Agent 转成 GLB 再发，MeshCue 不因此接 FBX。
标记要不要带时间／帧，是这版真正要设计的；若它改了标注契约，正是跳 major 的理由。

**未排版本**：相机 up 进提交、标记加文字（原记「1.4.0 的料」，1.4.0 已定为 GLB 范围）。
...
> **仍开**：相机 up 进提交、标记加文字 —— 都要往提交里加字段，是 minor；版本未排（见顶部版本规划）。
