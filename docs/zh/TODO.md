...
- **C（检查更新）—— 🟢 前两条前置已满足，可以开发**（原「换仓并转公开之后再做」已到期）。
  上游改读 **GitHub Release**（已拍板，见上）→ 写进 state → `/api/state` 带给页面 → 点击回传。
  前置三条，缺一条这功能就是个假的：① 仓库已公开 —— **✅ 2026-09-17 已转 public，
  `/releases/latest` 未认证可读，实测过**；② 地址已定且已开始打 Release —— **✅ `lzyling/meshcue`，
  v1.0.0 与 v1.0.1 都在，推 tag 即自动发版**；③ 「回传的触发机制」那条搁置项要有结论 —— **仍未解决，
  这是现在唯一挡着的一条**。
...
## 📋 下一批 · 1.0.1 之后（等 Kelven 说开工，**按新规矩批量处理，不逐条发补丁版本**）
...
- 🔴 **`inspect` 报的版本是磁盘 manifest，不是加载进内存的那份代码。**
