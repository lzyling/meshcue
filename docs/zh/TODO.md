...
Release。对策是流程规定 —— **产物可装了才打 Release**，这条本来就该有。

**远期留意**：两者其实回答的是**不同安装路径**的问题。OpenClaw 用户从 git 装 → GitHub 对；
将来 `npx meshcue-mcp` 的用户从 npm 装 → 对他们而言 npm 才对。真到两个渠道并存那天，
诚实的做法是**版本源跟随「这一份是怎么装上来的」**，而不是全局写死一个。今天不做。
