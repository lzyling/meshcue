...
**下一步：Kelven 人工测试 0.11.1。已安装；manager 那一层要等他重启 Gateway 才生效。**

0.11.1 是 0.11.0 的审阅修正版 —— 在他开始测试之前做的一轮场景推演，改掉 6 条。
最要紧的一条：**`open` 一个仍在运行的项目，原本一个「算使用」的路由都碰不到**，
于是一个闲置 23 小时的项目可以被重开、把地址交给人，然后在一小时内自己关掉。
