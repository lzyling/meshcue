...
授权核心（2026-09-10 15:32用户已认可）：入场许可15分钟、单次使用；每次新发放立即作废上一个未使用许可，已进入的浏览器不被踢出；浏览器会话自许可兑换成功起绝对有效期60分钟，授权轮换不续期、不清草稿、不解除审阅锁。进程重启／显式撤销／来源更换使相应授权失效。授权仅存在内存；会话 cookie 为 HttpOnly／SameSite=Strict，当前 HTTP LAN 不是 TLS。
